...
export async function updateProgress(args: UpdateProgressArgs): Promise<string> {
  const { workspaceRoot, cycleNumber, taskNumber, sessionDate, sessionDuration, sessionNotes } = args;
  ...
  try {
    ...
    if (taskMatches) {
      ...
      content = content.replace(
        ...
        `**Completed**: ${completed}/${total} tasks (${Math.round((completed / total) * 100)}%)`
      );
    ...
    if (sessionDate && sessionDuration) {
      ...
      if (sessionLogIndex !== -1) {
        ...
        if (tableStart !== -1) {
          ...
          const notStartedPattern = /\| -    \| -        \| -               \| Not started \|/;
          if (notStartedPattern.test(content)) {
            // Replace "Not started" row
            content = content.replace(
              notStartedPattern,
              `| ${sessionDate} | ${sessionDuration} | ${taskNumber || '-'} | ${sessionNotes || '-'} |`
            );
          } else {
            // Add new row after divider
            const insertPosition = content.indexOf('\n', tableStart) + 1;
            const newRow = `| ${sessionDate} | ${sessionDuration} | ${taskNumber || '-'} | ${sessionNotes || '-'} |\n`;
            content =
              content.substring(0, insertPosition) + newRow + content.substring(insertPosition);
          }
        }
      }
    }
